...
describe("TypedText Component", () => {
  test("renders and animates text correctly", async () => {
    ...
    await waitFor(
      ...
      { timeout: initialDelay + phrase.length * 100 + 500 }
    ); // Délai initial + durée de l'animation + un petit buffer pour les délais
  });
  ...
  test("does not display text before the initial delay", async () => {
    ...
    await waitFor(
      ...
      { timeout: initialDelay - 100 }
    ); // Attends juste avant le délai initial
  });
});
