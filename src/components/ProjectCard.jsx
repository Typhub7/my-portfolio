...
const ProjectCard = ({ title, image, githubLink, demoLink, description,badgeText  }) => {
  return (
    <div className="relative bg-white shadow-md rounded-lg overflow-hidden h-80"> {/* Ajustez la hauteur ici si nécessaire */}
      ...
      {badgeText && (
        <div className="absolute top-14 right-[-1rem] transform rotate-45 translate-x-8 -translate-y-2 text-center bg-main1 text-black text-sm px-2 py-1 shadow-md w-52">
          {badgeText}
        </div>
      )}
